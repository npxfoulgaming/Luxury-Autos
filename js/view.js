...
function getCanonicalVehiclePath(pathname) {
    ...
    /*
     * IMPORTANT:
     * Keep the actual canonical page path
     * unchanged so scrolling/layout behavior
...
function buildVehicleUrl(model) {
    ...
    /*
     * Add the slash ONLY to the URL used
     * for vehicle hash links.
     *
     * Do NOT change getCanonicalVehiclePath()
     * itself, because that would also change
     * the page's canonical pathname and could
     * affect scrolling/layout behavior.
     */
    if (
        path !== "/" &&
        !path.endsWith("/")
 ...
 *
 * IMPORTANT:
 * Keep the actual page pathname without
 * the extra slash so the original scrolling
